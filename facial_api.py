...
+ Integrasi Dashboard API (CRM & Blog).
"""
...
# ============================================================
#  DATABASE SETUP (IMPORT DARI FILE TERPISAH)
# ============================================================
# Mengimpor dari database.py untuk menghindari Circular Import
...
SMTP_PORT = 587
# Ambil dari Env, jika tidak ada pakai default lokal
...
app = FastAPI(
    ...
    version="2.5.0",
    description="Backend analisis kulit dengan YOLO + Notifikasi Email + DB Storage + Dashboard API."
)

app.add_middleware(
    CORSMiddleware,
    allow_origins=["mirrasense.my.id"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# --- IMPORT ROUTER DASHBOARD (CRM & BLOG) ---
# Aman diimpor karena sudah tidak ada circular import
from dashboard_api import router as dashboard_router
app.mount("/blog_image", StaticFiles(directory="blog_image"), name="blog_image")
app.include_router(dashboard_router) 
print("[INIT] Dashboard API Router loaded successfully.")
...
def generate_produk_rekomendasi(main_concerns, severity_map, uv_level="medium"):
    output = {"produk_kulit": {}, "rekomendasi_uv": None}
    ...
    for label in main_concerns:
        ...
        produk = REKOMENDASI_PRODUK[label].get(sev) or REKOMENDASI_PRODUK[label].get("ringan", [])
...
def compute_severity_percentage(conf, skin_r, contrast, w, h, img_w, img_h):
    ...
    score = (conf * 0.45 + skin_r * 0.15 + contrast_norm * 0.20 + size_norm * 0.20)
    return round(float(np.clip(score, 0.0, 1.0)) * 100.0, 2)

def severity_from_percentage(pct):
    if pct < 20: return "sangat ringan"
    elif pct < 40: return "ringan"
    elif pct < 60: return "sedang"
    elif pct < 80: return "berat"
    return "sangat berat"
...
def wbf_fusion(boxes, labels, confs, iou_thr=IOU_MERGE):
    fused = []
    ...
    for i in range(len(boxes)):
        ...
        c = 1.0
        for k in group: c *= (1.0 - confs[k])
        c = 1.0 - c
        fused.append((avg.tolist(), labels[i], float(c)))
    return fused

def soft_nms(boxes, labels, confs, iou_thr=SOFT_NMS_IOU, decay=SOFT_NMS_DECAY):
    if not boxes: return []
...
def add_blob_heatmap(heat_img, box, severity, weight=1.0, skin_mask=None):
    x1, y1, x2, y2 = map(int, box)
    ...
    blob = np.stack([gauss * b, gauss * g, gauss * r], axis=-1)
    heat_img[y1:y2, x1:x2, :] += blob

def finalize_heatmap(base_img, heat_raw):
    maxv = float(heat_raw.max())
    if maxv <= 0: return base_img.copy()
...
def run_predict(source):
    return model.predict(source=source, conf=BASE_CONF, imgsz=IMG_SIZE, device=DEVICE, half=(DEVICE != "cpu"), augment=False, verbose=False)
...
def analyze_image_bytes(image_bytes: bytes, pose: str, session_id: str) -> Dict[str, Any]:
    ...
    fused = wbf_fusion(all_boxes, all_labels, all_confs, IOU_MERGE) if all_boxes else []
    f_boxes, f_labels, f_confs = list(zip(*fused)) if fused else ([],[],[])
    snms = soft_nms(list(f_boxes), list(f_labels), list(f_confs)) if fused else []

    ts = datetime.now().isoformat()
    detections = []
    summary_conf, summary_pct = {}, {}
    overlay = img.copy()
    heat_raw = np.zeros_like(img, np.float32)
    severity_counts = {"sangat ringan": 0, "ringan": 0, "sedang": 0, "berat": 0, "sangat berat": 0}
    ...
    for k in sorted(summary_conf.keys()):
        ...
        summary_stats.append({"label": k, "count": len(confs), "avg_confidence": avg_conf, "severity": sev, "avg_severity_percentage": avg_pct})
    ...
    with open(det_path, "w", encoding="utf-8") as f:
        json.dump({"pose": pose, "timestamp": ts, "detections": detections, "summary": summary_stats, "severity_counts": severity_counts}, f, ensure_ascii=False, indent=2)

    cv2.imwrite(os.path.join(out_dir, "overlay.jpg"), overlay)
    cv2.imwrite(os.path.join(out_dir, "heatmap.jpg"), heatmap_img)

    return {
        "pose": pose, "detections": detections, "summary": summary_stats, "severity_counts": severity_counts,
        "files": {"folder": out_dir, "overlay": "overlay.jpg", "heatmap": "heatmap.jpg", "detections_json": "detections.json"},
        "overlay_image": {"base64": encode_image_to_base64(overlay), "mime": "image/jpeg"},
        "heatmap_image": {"base64": encode_image_to_base64(heatmap_img), "mime": "image/jpeg"},
    }
...
HIGH_CONCERN_LABELS = {"Jerawat", "Papula", "Pustula", "Nodul Jerawat", "Jerawat Kistik", "Bekas Jerawat", "Folikulitis", "Keloid", "Siringoma", "Kutil Datar", "Flek Hitam", "Kemerahan Kulit"}

def evaluate_skin_health(all_detections: List[Dict[str, Any]]) -> Dict[str, Any]:
    if not all_detections:
        return {"is_healthy": True, "health_status": "sehat", "health_score": 100.0, "severity_score": 0.0, "main_concerns": [], "stats": {"total_detections": 0, "high_concern_count": 0, "severe_lesions_count": 0, "per_label_counts": {}}, "message": "Tidak ada temuan signifikan, kulit tampak sehat."}
    ...
    return {
        "is_healthy": is_healthy, "health_status": health_status, "health_score": round(health_score, 2), "severity_score": round(severity_score, 2),
        "main_concerns": main_concerns,
        "stats": {"total_detections": total_detections, "high_concern_count": high_concern_count, "severe_lesions_count": severe_lesions_count, "per_label_counts": label_counts},
        "message": msg,
    }
...
def build_severity_map(all_dets):
    ...
    for d in all_dets:
        lbl = d.get("label"); sev = d.get("severity")
        if lbl and sev:
            old = severity_map.get(lbl)
            if (not old) or (rank.index(sev) > rank.index(old)): severity_map[lbl] = sev
    return severity_map
...
def register_user(user: UserCreate, db: Session = Depends(get_db)):
    try:
        ...
        if existing_user:
            ...
            existing_user_by_phone = db.query(User).filter(User.tel == user.tel).first()
            if existing_user_by_phone:
                existing_user_by_phone.name = user.name
                existing_user_by_phone.email = user.email
                action = "diperbarui via telepon"
                db_user = existing_user_by_phone
            else:
                new_user = User(name=user.name, email=user.email, tel=user.tel, created_at=datetime.now())
                db.add(new_user)
                action = "dibuat"
                db_user = new_user
        db.commit()
        db.refresh(db_user)
        print(f"[DB] User {db_user.email} {action}.")
        return {"success": True, "message": f"User berhasil {action}", "data": {"id": db_user.id, "name": db_user.name, "email": db_user.email, "tel": db_user.tel, "created_at": db_user.created_at.isoformat()}}
    except Exception as e:
        db.rollback()
        print(f"[ERROR] Register DB Error: {e}")
...
async def analyze_face(
    ...
    if email: background_tasks.add_task(send_email_notification, email, name, session_id, eva, processed_poses_keys)
    if phone: background_tasks.add_task(send_wa_notification, phone, name, session_id, eva)

    response = {
        "success": True, "processed_at": datetime.now().isoformat(), "session_id": session_id,
        "user": {"name": name, "email": email, "phone": phone},
        "poses": poses_output, "health_evaluation": eva
    }
...
async def skin_health(session_id: str):
    ...
    if not os.path.exists(session_path): raise HTTPException(status_code=404, detail="Session not found")
    all_detections = []
    for pose in ["left", "center", "right"]:
        det = load_json(os.path.join(session_path, pose, "detections.json"))
        if det: all_detections.extend(det.get("detections", []))
    eva = evaluate_skin_health(all_detections)
    severity_map = build_severity_map(all_detections)
    return {"session_id": session_id, "success": True, **eva, "rekomendasi_produk": generate_produk_rekomendasi(eva["main_concerns"], severity_map)}
